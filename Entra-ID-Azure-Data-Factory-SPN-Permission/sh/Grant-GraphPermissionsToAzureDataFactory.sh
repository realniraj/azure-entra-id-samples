...
# --- 4. Check for Existing Permission (Idempotency) ---
echo "Checking if permission is already assigned..."
...
EXISTING_ASSIGNMENT=$(az rest \
  ...
  --query "value[?appRoleId=='${SITES_SELECTED_ROLE_ID}' && resourceId=='${GRAPH_SP_ID}']" \
  --output tsv)

if [[ -n "${EXISTING_ASSIGNMENT}" ]]; then
  echo "✅ Permission 'Sites.Selected' is already assigned to '${DATA_FACTORY_NAME}'. No action needed."
  exit 0
fi

echo "Permission not found. Proceeding with assignment..."

# --- 5. Grant the Permission ---
echo "Constructing request body for assignment..."
JSON_BODY=$(jq -n \
  --arg principalId "$ADF_PRINCIPAL_ID" \
  --arg resourceId "$GRAPH_SP_ID" \
  --arg appRoleId "$SITES_SELECTED_ROLE_ID" \
  '{principalId: $principalId, resourceId: $resourceId, appRoleId: $appRoleId}')

echo "Assigning 'Sites.Selected' to ADF '${DATA_FACTORY_NAME}' via Microsoft Graph API..."
...
echo "✅ Successfully assigned 'Sites.Selected' permission to '${DATA_FACTORY_NAME}'."
